...
**실습과제 2** 에서는 tri1, tri2 가 main 함수 내부의 **지역 객체**이므로 main 이 실행된 후 선언문이 실행되는 시점에 생성자가 호출된다. 

**실습과제 3** 에서는 globalTri1, globalTri2 가 **전역 객체**이므로 프로그램이 시작되자마자 main 함수 실행 전에 생성자가 먼저 호출된다. 
